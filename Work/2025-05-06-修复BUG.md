...
### 问题描述
当AI自动选择目标时，没有显示选中效果（紫色外圈），导致玩家无法直观地看到当前选中的目标是哪个。

### 问题分析
1. 当用户手动点击豆豆时，会调用`EntityRenderer.setSelectedEntity`方法显示选中效果
2. 但当AI自动选择目标时，没有调用这个方法，所以没有显示选中效果
3. 需要在战斗后端和视图层之间建立通信机制，当英雄目标变化时通知视图层显示选中效果

### 解决方案
1. 在`EventTypes.ts`中添加新的事件类型`HERO_TARGET_CHANGED`
2. 创建新的事件接口文件`HeroTargetChanged.ts`
3. 修改`Hero.ts`中的`setTargetId`方法，使其触发`HERO_TARGET_CHANGED`事件
4. 在`BattleManager.ts`中确保在创建英雄时设置事件管理器
5. 在`EventHandlers.ts`中添加对`HERO_TARGET_CHANGED`事件的监听，并在收到事件时设置选中效果

### 修改内容
1. 在`EventTypes.ts`中添加新的事件类型：
```typescript
// 英雄事件
...
3. 修改`Hero.ts`中的`setTargetId`方法：
```typescript
...
4. 在`BattleManager.ts`中修改`createHero`方法：
```typescript
// 设置伤害管理器、实体管理器和事件管理器
hero.setDamageManager(this.damageManager);
hero.setEntityManager(this.entityManager);
...
5. 在`EventHandlers.ts`中添加对`HERO_TARGET_CHANGED`事件的监听：
```typescript
// 英雄目标变化事件
bindEventHandler(EventType.HERO_TARGET_CHANGED, this.onHeroTargetChanged);
```

6. 在`EventHandlers.ts`中添加`onHeroTargetChanged`方法：
```typescript
/**
 * 英雄目标变化事件处理
 * @param event 事件数据
...
### 测试结果
修改完成后，当AI自动选择目标时，现在能够正确显示选中效果（紫色外圈），使玩家能够直观地看到当前选中的目标是哪个。这提高了游戏的可视化反馈，使玩家更容易理解游戏状态。
